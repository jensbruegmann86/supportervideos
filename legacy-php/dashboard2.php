...
// Alle Videos laden
$stmt = $pdo->query("SELECT * FROM event_video WHERE approved = 0 ORDER BY upload_time DESC");
$videos = $stmt->fetchAll();
$videoCount = count($videos);

// Anzahl der hochgeladenen Videos zählen
...
<p><strong>Offene Videos zur Prüfung:</strong> <?= $videoCount ?></p>
...
<div class="card mb-3">
    <div class="card-body">
        <h5 class="card-title">BIB: <?= htmlspecialchars($bib) ?> | Video #: <?= $count ?> | Freigabe: <?= $video['approved'] ? 'Ja' : 'Nein' ?></h5>
        <?php if ($remoteFile): ?>
