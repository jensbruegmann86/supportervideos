...
// Replaces upload.php: shows the selected participant and handles the direct
// browser -> Supabase Storage upload (see /api/upload/sign + /complete).
export default function UploadPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!file) {
      setMessage("Bitte ein Video ausw\u00e4hlen.");
      return;
    }
    if (!agree) {
      setMessage("Bitte den Nutzungsbedingungen zustimmen.");
      return;
    }
    if (file.size > MAX_MB * 1024 * 1024) {
      setMessage(`Datei ist gr\u00f6\u00dfer als ${MAX_MB} MB.`);
      return;
    }
    ...
    try {
      ...
      if (!signRes.ok) throw new Error(signData.error || "Upload konnte nicht vorbereitet werden.");
      ...
      if (!completeRes.ok) throw new Error(completeData.error || "Speichern fehlgeschlagen.");

      setStatus("done");
      setMessage("Dein Video wurde erfolgreich hochgeladen!");
    } catch (err) {
      setStatus("error");
          ...
          {participant && (
            <div className="btn btn-lg btn-light border border-gkm text-start position-relative mb-3 w-100">
              ...
              <span className="text-gkm-sm">
                {participant.bib} |{" "}
                {participant.race === 1 ? "Marathon" : participant.race === 2 ? "Halbmarathon" : ""}
              </span>
            </div>
          )}
